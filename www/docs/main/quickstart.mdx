...
Here's the file structure we'll be building. By convention, we recommend separating tRPC initialization, router definition, and server setup into distinct files to prevent cyclic dependencies:

```
.
├── server/
│   ├── db.ts          # an imaginary database (can be whatever you like)
...
Now that we have defined our router, we can serve it. tRPC has first-class [adapters](/docs/server/adapters) for many popular web servers. To keep it simple, we'll use the [`standalone`](/docs/server/adapters/standalone) Node.js adapter here.
...
Links in tRPC are similar to links in GraphQL, they let us control the data flow to the server. In the example above, we use the [httpBatchLink](/docs/client/links/httpBatchLink), which automatically batches up multiple calls into a single HTTP request. For more in-depth usage of links, see the [links documentation](/docs/client/links).
...
| [Server Adapters](/docs/server/adapters) | Express, Fastify, and more |
| [Transformers](/docs/server/data-transformers#using-superjson) | Use superjson to retain complex types like `Date` |
